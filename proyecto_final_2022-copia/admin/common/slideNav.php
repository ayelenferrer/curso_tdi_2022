 <!-- START LEFT SIDEBAR NAV / Nav de la izquierda -->
 <aside id="left-sidebar-nav">
        <ul id="slide-out" class="side-nav fixed leftside-navigation">
          <li class="user-details pink">
            <div class="row">
              <div class="col col s4 m4 l4">
                <i class="medium material-icons">account_circle</i>
              </div>
              <div class="col col s8 m8 l8">
                <a class="btn-flat dropdown-button waves-effect waves-light white-text profile-btn" href="#" data-activates="profile-dropdown-nav">User<i class="mdi-navigation-arrow-drop-down right"></i></a>
                <p class="user-roal">Administrador</p>
              </div>
            </div>
          </li>
          <li class="no-padding">
            <ul class="collapsible" data-collapsible="accordion">
              <li class="bold">
                <a href="../usuarios/administradores.php" class="waves-effect waves-cyan">
                  <i class="material-icons">home</i>
                  <span class="nav-text">Inicio</span>
                </a>
              </li>
              <li class="bold">
                <a href="../pedidos/tablaPedidos.php" class="waves-effect waves-cyan">
                  <i class="material-icons">shopping_cart</i>
                  <span class="nav-text">Ventas</span>
                </a>
              </li>
              <li class="bold">
                <a href="../productos/tablaProductos.php" class="waves-effect waves-cyan">
                  <i class="material-icons">apps</i>
                  <span class="nav-text">Productos</span>
                </a>
              </li>
              <li class="bold">
                <a href="../categorias/tablaCategorias.php" class="waves-effect waves-cyan">
                  <i class="material-icons">storage</i>
                  <span class="nav-text">Categorías</span>
                </a>
              </li>
            </ul>
            <a href="#" data-activates="slide-out" class="sidebar-collapse btn-floating btn-medium waves-effect waves-light hide-on-large-only">
              <i class="material-icons">menu</i>
            </a>
      </aside>
      <!-- END LEFT SIDEBAR NAV-->